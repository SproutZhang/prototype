...
import { scenarioFreezeT } from '../i18n/scenarioStrings'

type ScenarioFreezeModalProps = {
  open: boolean
  locale: AppLocale
  scenarioName: string
  onClose: () => void
...
export function ScenarioFreezeModal({
  open,
  locale,
  scenarioName,
  onClose,
  onConfirm,
}: ScenarioFreezeModalProps) {
  const headingId = useId()
  const descId = useId()
  const text = scenarioFreezeT(locale)
  ...
  const message = text.modalMessage.replace('{name}', scenarioName)
