...
type ScenarioFreezeRunModalProps = {
  open: boolean
  locale: AppLocale
  scenarioName: string
  onClose: () => void
...
export function ScenarioFreezeRunModal({
  open,
  locale,
  scenarioName,
  onClose,
  onConfirm,
}: ScenarioFreezeRunModalProps) {
  ...
  const message = scenarioT(locale, 'freezeRunModalMessage').replace('{name}', scenarioName)
      ...
      <div className="scenario-freeze-run-modal__panel">
        <header className="scenario-freeze-run-modal__header">
          <h2 id={headingId} className="scenario-freeze-run-modal__title">
            {scenarioT(locale, 'freezeRunModalTitle')}
          </h2>
          <button
            type="button"
        ...
        <div className="scenario-freeze-run-modal__footer">
          ...
          <button type="button" className="scenario-freeze-run-modal__confirm" onClick={onConfirm}>
            {scenarioT(locale, 'freezeRunModalConfirm')}
          </button>
        </div>
      </div>
    </div>,
